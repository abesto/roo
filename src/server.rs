    ...
    loop {
        tokio::select! {
                ...
                result = listener.accept() => {
                    ...
                    if let Some(uuid) = do_login_command(&lua, &not_logged_in_uuid, line_rx.clone(), notify_tx.clone(), db.clone()).await {
                        {
                            let mut db = db.write().unwrap();
                            db.delete(&not_logged_in_uuid).unwrap();
                        }
                        lua.load(&format!("player = db[{:?}]", uuid.to_string())).exec()?;
...
async fn eval(lua: &Lua, chunk_name: String, lua_code: String, notify_tx: &mpsc::Sender<String>) {
    ...
    // gc_collect() docs says it may be necessary to run it twice
    for _ in 1..=2 {
        if let Err(e) = lua.gc_collect() {
            if let Err(e) = notify_tx.send(e.to_string()).await {
                println!("spawn_eval_task/gc -> notify_tx.send: {}", e);
            }
        }
    }

    println!("eval-done: {}", chunk_name);
}
    ...
    tokio::task::spawn_local(async move {
        loop {
            ...
            let res = if let Some(stripped) = line.strip_prefix(';') {
                ...
                execute_verb(db.clone(), &player_uuid, line)
            };
            match res {
                Ok((chunk_name, lua_code)) => eval(&lua, chunk_name, lua_code, &notify_tx).await,
                Err(s) => {
...
fn execute_verb(
    ...
    let location: Option<&Object> = {
        lock.get_old(player_uuid)
            .unwrap()
            .location()
            .and_then(|l| lock.get_old(&l).ok())
    };
    // Find what
    let (this, verb) = match first_matching_verb(
        &lock,
        &command,
        vec![Some(lock.get_old(player_uuid).unwrap()), location],
    ) {
        Ok(Some(x)) => x,
...
fn move_notify_channel(
    ...
    lock.insert(new, val);
    println!("Moved notify_tx from {} to {}", old, new);
}
...
async fn do_login_command(
    ...
    loop {
        ...
        let full_code = format!("player = db[{:?}]; return {}", uuid.to_string(), short_code);

        // TODO maybe replace this with eval()
        println!("eval-start: do_login_command\n{}", full_code.trim());
        let retval = lua
            .load(&full_code)
            .set_name(&short_code)
            .unwrap()
            .eval::<LuaValue>();
        println!("eval-done: do_login_command");
