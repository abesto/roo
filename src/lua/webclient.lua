...
return {name, code, "@program %s:%s %s" % {object.uuid, vname, vargs}};
]])
...
local new = args[1]
local spec = this:parse_invoke(args)
if type(spec) == "table" and not M.Error:class_of(spec) then
  local info = this:local_editing_info(spec)
...
local argspec = M.listdelete(vref, 1)
if #argspec > 0 then
  local pas = S.code_utils:parse_argspec(argspec)
  if type(pas) == "table" then
    if pas[2] and #pas[2] > 0 then
      player:tell{"I don't understand \"", S.string_utils:from_list{pas[2], " "}, "\""}
      return
    end
    argspec = pl.List(pas[1]):extend{"none", "none"}:slice(1, 3)
    argspec[2] = S.code_utils:full_prep{argspec[2]} or argspec[2]
  else
    player:tell{pl.pretty.write(pas)}
    return
  end
end
player:tell{M.toliteral(spec)}
...
if not S.command_utils:object_match_failed{object, spec[1]} then
  ...
  if M.Error:class_of(code) then
    player:tell{(code ~= M.E_VERBNF) and code or "That object does not define that verb", argspec and " with those args." or "."}
    return code
  else
...
M.set_task_perms(player)
local status, code = pcall(function() return M.verb_code(args[1], args[2], not player:edit_option("no_parens")) end)
print("fetch", status, code)
return status and code or ""
]])
